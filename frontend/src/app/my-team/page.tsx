...
function MyTeamContent() {
  ...
  const maxRoster = Number(process.env.NEXT_PUBLIC_TEAM_MAX_ROSTER || 5)
  ...
  return (
    <div className="p-8">
      ...
      <div className="mt-4">
        <label className="block text-sm font-medium">Select fighters (max {maxRoster})</label>
              ...
              <input
                ...
                onChange={() => {
                  if (!selectedFighters.includes(f.id)) {
                    if (selectedFighters.length >= maxRoster) {
                      setToast({ message: `Max roster size is ${maxRoster}`, type: 'error' })
                      return
                    }
        ...
        <div className="mt-3">
          <div className="text-sm text-slate-600 mb-2">Selected: {selectedFighters.length} / {maxRoster}</div>
          <button
            onClick={async () => {
            ...
            disabled={!selectedLeague || selectedFighters.length === 0 || selectedFighters.length > maxRoster}
          >
            Create Team
          </button>
        </div>
